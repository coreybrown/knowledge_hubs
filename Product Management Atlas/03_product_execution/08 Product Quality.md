...
Quality is **not QA**. QA is a process — a phase, a checklist, sometimes a team. Quality is a *standard of judgment* the PM carries through the entire lifecycle. The PM doesn't personally test every device, but the PM owns the *bar*: deciding what's a launch blocker and what isn't, weighing impact against effort, and being accountable when something ships broken. Reframing it from "assurance" (a gate at the end) to "quality" (a judgment throughout) is the whole point.

The definition's range is deliberately wide — **technical, functional, AND business quality, across all devices, points of sale, and use cases.** In my B2C mobile world that's brutal: iOS, Android, web, tablet, every market, every edge case. Business quality is the one juniors forget — a feature can be bug-free and still be low quality if it doesn't move the [[Outcomes Over Outputs|outcome]] it was built for.
...
> Notice the jump: junior levels *do* quality (QA a feature, triage bugs). Senior levels *create the conditions* for quality — coaching, stakeholder cover for risky calls, and ultimately a culture where the team takes pride in the bar without being told. That shift from doing to culture-building is the real promotion.
...
Quality is being redefined for AI products in a way that doesn't apply to deterministic software. When the same input can produce different outputs, "bug-free" stops being a coherent bar — you can't enumerate correct behavior. The replacement is **evals**: OpenAI's Kevin Weil calls writing them "a core skill for product managers," the new equivalent of unit tests. Quality for a probabilistic feature means defining what good looks like in a rubric, building a golden dataset, and measuring against it — and accepting you'll never promise zero hallucinations, only reliability designed *around* them. This is the most consequential shift to this competency in the whole model. See [[Writing Evals for AI Products]] and [[Designing for Trust and Probabilistic Systems]].

## Continue Reading
- [[Sound Product Decision-Making]] — the first behavior: qual + quant, reversible vs irreversible, owning the call.
- [[Risk Identification and Mitigation]] — the second behavior: classify, contingency, communicate, monitor.
- [[The Impact-Likelihood Matrix]] — the 2×2 behind both behaviors.
- [[Writing Evals for AI Products]] — what the quality bar becomes for probabilistic systems.
