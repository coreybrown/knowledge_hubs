...
- Decide, with evidence: **iterate, double down, or sunset.** [[theScore Product Principles|"Don't beat a dead horse"]] — a feature that didn't move the number deserves an honest look, not loyalty.
...
The launch *mechanics* are stable — flags, phased rollout, and canaries predate AI and still apply. What changes is launching *AI features*, where "ready" can't mean "bug-free" because behavior is probabilistic. Launch readiness for an AI feature adds an **eval gate** — does it clear the rubric on the golden set ([[Writing Evals for AI Products]]) — plus online monitoring of model behavior, not just error rates, and explicit human-in-the-loop checkpoints for high-stakes actions. The data-flywheel logic also reframes launch: usage generates the feedback data that improves the model, so the rollout *is* part of the training loop. See [[Designing for Trust and Probabilistic Systems]].

## Continue Reading
- [[Outcomes Over Outputs]] — why post-launch measurement is the only thing that makes a launch "done."
- [[Experimentation and A-B Testing]] — proving the outcome moved, not just assuming it.
- [[Risk Identification and Mitigation]] — rollback paths and phased rollout as risk controls.
- [[The 10-50-90 Execution Framework]] — launch readiness as the 90% confidence gate.
- [[Product Analytics and Metrics]] — instrumenting the success metric before you ship.
