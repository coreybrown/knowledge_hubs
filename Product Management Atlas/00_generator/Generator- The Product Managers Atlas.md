...
## Source priority
1. **Corey's own frameworks (source #1)**, `sources/SOURCE_DIGEST.md`. The 4-pillar / 12-competency *Product Level Competency Matrix* is the **spine** (domains 03 to 06 are it, verbatim). The *PM Evaluation Sandbox*, the *theScore Career Journey* (score-gated leveling), the *Uniform 10/50/90* execution framework, and *theScore Product Principles* are Corey's and should be featured as his.
...
## The 12 competency-note template (domains 03 to 06)
...
5. **How AI is changing it**, 2 to 4 sentences grounded in AI_AND_PM.md.
...
- 350 to 700 words (index/glossary/matrix notes may run longer).
- Tables + Obsidian callouts (`> [!note] > [!tip] > [!warning] > [!example]`) where they help.
- 6 to 15 `[[wikilinks]]`, each an exact registry name; cross-domain links encouraged; `[[Name|display]]` for inline phrasing; inside tables escape the pipe as `\|`. Keep each link on one line.
- End with `## Continue Reading`: 3 to 5 related wikilinks with short descriptions.
